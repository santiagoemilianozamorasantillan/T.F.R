...
const ShoppingCart = ({ isCartOpen, setIsCartOpen }) => {
  ...
  // Listen for custom event to open cart from anywhere (e.g., toast notifications)
  useEffect(() => {
          ...
          >
                  ...
                  <div key={item.variant.id} className="flex items-center gap-4 bg-card border border-border p-3 rounded-xl">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="flex items-center border border-border rounded-lg overflow-hidden">
                        <Button onClick={() => updateQuantity(item.variant.id, Math.max(1, item.quantity - 1))} size="sm" variant="ghost" className="px-2 h-8 text-card-foreground hover:bg-muted rounded-none">-</Button>
                        <span className="px-3 text-sm font-medium text-card-foreground">{item.quantity}</span>
                        <Button onClick={() => updateQuantity(item.variant.id, item.quantity + 1)} size="sm" variant="ghost" className="px-2 h-8 text-card-foreground hover:bg-muted rounded-none">+</Button>
                      </div>
                      <Button onClick={() => removeFromCart(item.variant.id)} size="sm" variant="ghost" className="text-destructive hover:text-destructive/90 hover:bg-destructive/10 text-xs h-6 px-2">Remove</Button>
                    </div>
                  </div>
                ))
              )}
            ...
            {cartItems.length > 0 && (
              <div className="p-6 border-t border-border bg-muted/30">
                ...
                <Button onClick={handleCheckout} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold py-6 text-lg rounded-xl shadow-lg hover:shadow-xl transition-all active:scale-[0.98]">
                  Proceed to Checkout
                </Button>
