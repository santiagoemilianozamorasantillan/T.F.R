...
const Shop = ({ setIsCartOpen }) => {
  const location = useLocation();
  const navigate = useNavigate();

  useEffect(() => {
    // Check if we navigated here with the intent to open the cart
    if (location.state?.openCart && setIsCartOpen) {
      setIsCartOpen(true);
      
      // Clear the state so the cart doesn't reopen on page refresh
      navigate(location.pathname, { replace: true, state: {} });
    }
  }, [location, navigate, setIsCartOpen]);
